...
reset = word_to_vec_map["strong"]
change = word_to_vec_map["personality"]

print("cosine_similarity(father, mother) = ", cosine_similarity(father, mother))
print("cosine_similarity(father, mother) = ", cosine_similarity(reset, change))
...
# triads_to_try = [('boy', 'superman', 'girl')]
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = (np.sqrt(np.abs(1 - np.linalg.norm(mu_orth))) ) * (e_w1B - mu_B )/(np.abs((e_w1 - mu_orth) - mu_B))
    corrected_e_w2B = (np.sqrt(np.abs(1 - np.linalg.norm(mu_orth))) ) * (e_w2B - mu_B )/(np.abs((e_w2 - mu_orth) - mu_B))
